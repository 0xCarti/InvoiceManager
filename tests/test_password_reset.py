...
def test_password_reset_flow(client, app, monkeypatch):
    ...
    with app.app_context():
        user = User(email='reset@example.com', password=generate_password_hash('old'), active=True)
        db.session.add(user)
        db.session.commit()

    client.post('/auth/reset', data={'email': 'reset@example.com'}, follow_redirects=True)
    assert 'message' in sent
    m = re.search(r'/auth/reset/([^\s]+)', sent['message'].get_content())
    token = m.group(1)

    client.post(f'/auth/reset/{token}', data={'new_password': 'newpass', 'confirm_password': 'newpass'}, follow_redirects=True)
...
def test_password_reset_unknown_email(client):
    response = client.post('/auth/reset', data={'email': 'missing@example.com'}, follow_redirects=True)
